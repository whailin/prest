        ...
        if (parserList == null) {
            ...
            for (ParserInterfaceAndFileList parserAndFiles : parserList) {
                ...
                if (thisOne == null) {
                	...
                	//separated by Ozan
                	Language tempLanguage = parserAndFiles.getParser().getLanguage();
                	ParseResult tempParseResult = new ParseResult(tempLanguage, thisOne);
                    parserResultList.add(tempParseResult);
                }
            }
    ...
    public static DataContext parseProject(IParser aParser,
        ...
        if (aParser != null && fileList != null) {
	    	...
	    	String classCsvFileName = ApplicationProperties.get("repositorylocation") + "\\" + projectName + "\\parse_results\\parseResult" + "_" + aParser.getLanguage().getLangName() + "_" + Calendar.getInstance().getTimeInMillis() + "CLASS.csv"; 
    	    	String methodCsvFileName = ApplicationProperties.get("repositorylocation") + "\\" + projectName + "\\parse_results\\parseResult" + "_" + aParser.getLanguage().getLangName() + "_" + Calendar.getInstance().getTimeInMillis() + "METHOD.csv";
                
                metrics = aParser.startExecution(fileNames, projectName, xmlFileName, packageCsvFileName,fileCsvFileName,classCsvFileName,methodCsvFileName);
            } catch (Exception e) {
                Logger.error(ParserExecutor.class.getName() + " " + "error in parsing " + projectName + " " + e.getMessage());
            }
    ...
    public static class ParserLanguageCheckBoxDialog extends JPanel implements
        ...
        public ParserLanguageCheckBoxDialog(
            ...
            for (ParserInterfaceAndFileList parserInterfaceAndFileList : parserList) {
                JCheckBox checkBox = new JCheckBox(parserInterfaceAndFileList.getParser().getLanguage().getLangName());
